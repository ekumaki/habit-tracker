...
export const HabitList: React.FC<HabitListProps> = ({
    ...
    const visibleHabits = habits.filter(habit => {
        const startDate = habit.startDate || format(habit.createdAt, 'yyyy-MM-dd');
        return startDate <= dateStr;
    });

    // Only enable reordering if all habits are visible to prevent data loss
    // (and maybe restrict to today for simplicity, but checking length is safer for data integrity)
    const isReorderEnabled = visibleHabits.length === habits.length;

    const handleDragEnd = (result: DropResult) => {
        if (!result.destination) return;

        const items = Array.from(visibleHabits);
    ...
    const renderHabitItem = (habit: Habit, index: number, provided?: any, snapshot?: any) => {
        ...
        return provided ? content : <div key={habit.id}>{content}</div>;
    };
